...
function ea(){ai=!ai;fetch('/api/enable_ai',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:rid,ai:ai})}).then(r=>r.json()).then(d=>{if(d.ok){up(d.game);sm(ai?'AI已启用':'AI已禁用',false);if(ai&&cur=='O')am();}else{sm('切换失败',true);}}).catch(e=>{sm('切换失败',true);});}
function am(){fetch('/api/ai',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:rid})}).then(r=>r.json()).then(d=>{if(d.ok){up(d.game);}else{sm('无法落子',true);}}).catch(e=>{sm('AI失败',true);});}
function re(){fetch('/api/reset',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:rid})}).then(r=>r.json()).then(d=>{if(d.ok){up(d.game);document.getElementById('gm').style.display='none';}else{sm('重置失败',true);}}).catch(e=>{sm('重置失败',true);});}
